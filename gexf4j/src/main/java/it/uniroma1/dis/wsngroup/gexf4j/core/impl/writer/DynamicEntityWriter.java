...
public abstract class DynamicEntityWriter<T extends Dynamic<?>> extends AbstractEntityWriter<T> {
	...
	protected void writeAttributes() throws XMLStreamException {
		...
		/** double timeformat */
		if(writerTimeType.equals(TimeFormat.DOUBLE)) {
			if (entity.hasStartDate()) {
				String startValue = toDouble(entity.getStartValue());
			...
			if (entity.hasEndDate()) {
				String endValue = toDouble(entity.getEndValue());
		...
		/** integer timeformat */
		if(writerTimeType.equals(TimeFormat.INTEGER)) {
			if (entity.hasStartDate()) {
		...
		/** date timeformat */
		if(writerTimeType.equals(TimeFormat.DATE)) {
			if (entity.hasStartDate()) {
		...
		/** dateTime timeformat */
		if(writerTimeType.equals(TimeFormat.XSDDATETIME)) {
			if (entity.hasStartDate()) {
